...
def sql_chain(question, limit = 10):
    sql_query = generate_sql_query(question)
    pattern = "<SQL>(.*?)</SQL>"
    matches = re.findall(pattern, sql_query, re.DOTALL)

    if len(matches) == 0:
        return "Sorry, LLM is not able to generate a query for you question"
    ...
    if truncated:
        answer += f"\n\nNote: Only the first {limit} results are shown. Please refine your query for more specific results."
    return answer
    

if __name__ == "__main__":
    question = "All products with rating higher than 4.5 and total_ratings greater than 50"
